...
class InstagramRegistration(object):
    ...
    def createAccount(self, username, password, email, name=''):
        ...
        data = json.dumps(
            OrderedDict([
                ...
                ('first_name', name),
                ('guid', self.uuid),
                ('device_id', SignatureUtils.generateDeviceId(hashlib.md5((username + password).encode()).hexdigest())),
                ('email', email),
                ('force_sign_up_code', ''),
    ...
    def getCsfrtoken(self):
        ...
        fetch = self.request('si/fetch_headers/', None)
        ...
        match = re.search(r'^Set-Cookie: csrftoken=([^;]+)', fetch[0].decode(), re.MULTILINE)
